#!/bin/bash
config_prefix="/usr/share/scan_config/icmp/first/"
...
taskset -c $cores_zmap $path_zmap""zmap -i $interface -G $gateway  -S $src_ip --probe-module=icmp_echoscan -r $scan_rate --sender-threads=$send_threads -c $icmp_cooloff -b $path_blacklist -e $seed --output-module=csv  -f "$out_fields" -m $out_icmp"meta.log" $block 2>$out_icmp"err.log" > $out_icmp"out.log"
