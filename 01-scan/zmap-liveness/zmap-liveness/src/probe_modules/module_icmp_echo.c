		...
		uint32_t len, uint32_t *src_ip, uint32_t *validation)
{
	...
	if (icmp_h->icmp_type == ICMP_TIMXCEED || icmp_h->icmp_type == ICMP_UNREACH) {
		//Bano
		//return 1; // process without checking validation
		...
		//src_ip = ip_inner->ip_dst.s_addr; //Bano
		...
		__attribute__((unused)) uint32_t len, fieldset_t *fs)
{
	...
	switch (icmp_hdr->icmp_type) {
		case ICMP_ECHOREPLY:
			...
			break;
		//Bano commenting out the rest, and dumping the rest
		// to a pcap file
		...
		case ICMP_UNREACH:
			...
			// Add inner_daddr
			ip_inner = (struct ip*) ((char *) icmp_hdr+8);
		...
		//Bano dummy entry for non-exho reply msgs
		/*
		default:
                        fs_add_string(fs, "classification", (char*) "other", 0);
                        fs_add_uint64(fs, "success", 0);
                        fs_add_null(fs, "inner_daddr");
			//pcap_dump(zconf.fp_icmp, zconf.ph, packet); //dump to pcap file
                        break;
		*/
	}
}
...
probe_module_t module_icmp_echo = {
	.name = "icmp_echoscan",
	.packet_length = 62,
	//.pcap_filter = "ip host 128.232.110.29 and (icmp and icmp[0]!=8)",
	.pcap_filter = "ip host 128.232.110.29 and (icmp and (icmp[0]==0 or icmp[0]==3 or icmp[0]==11))",
	.pcap_snaplen = 96,
	.port_args = 0,
