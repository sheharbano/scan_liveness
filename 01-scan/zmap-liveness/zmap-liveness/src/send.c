...
int send_run(sock_t st, shard_t *s)
{
	...
	uint32_t curr = shard_get_cur_ip(s);
        sprintf(tmp_buffer,"%"PRIu32,curr);
	push_back(tmp_buffer,zconf.queue_ips);
        zsend.sent_orig++;
        s->state.sent++;
	...
	while (1) {
		...
		if (zrecv.complete) {
			if( !zconf.should_retransmit || (zconf.should_retransmit && zsend.retransmit_done)) {
				s->cb(s->id, s->arg);
				break;
			}
			zsend.send_done=1;
		...
		if(zconf.mode_retransmit==0) {	
			// If original probes are done, but retransmissions
			// not finished, interleave with probes to local
			// loopback 127.0.0.1
			if(zconf.should_retransmit && zsend.send_done) {
				curr=16777343;
			}
			else {
				curr = shard_get_next_ip(s);
				// Push to queue
                                sprintf(buffer,"%"PRIu32,curr);
				push_back(buffer,zconf.queue_ips);
				// Update counters
				zsend.sent_orig++;
				s->state.sent++;

                                if((zsend.sent_orig%1000000)==0)
                                {
					lock_file(stderr);
					fprintf(stderr,"^\t%0.6lf\t%"PRIu32"\n",now(),curr);
					fflush(stderr);
					unlock_file(stderr);
                                }
			}
			// switch to retransmit
		...
		// Retransmit probes
			...
			if(zconf.should_retransmit && zsend.sent_orig < IP_RETRANSMIT_SIZE) {
                                curr=16777343;
                        }
			else {
				// If retransmission queue is not empty,
				// pop an address to probe
				if(!is_empty(zconf.queue_ips)) {
					// Pop an address
					node = pop_front(zconf.queue_ips);
					curr = (uint32_t)strtoul(node->data, &dummy, 10);
					...
					zsend.retransmitted++;
                                        ...
                                        if((zsend.retransmitted%1000000)==0)
                                        {
                                                lock_file(stderr);
                                                fprintf(stderr,"!\t%0.6lf\t%"PRIu32"\n",now(),curr);
                                                fflush(stderr);
				...
				// Retransmissions done; finished
				else {
					zsend.retransmit_done=1;
				}
			}
			// Switch to orig probes        
