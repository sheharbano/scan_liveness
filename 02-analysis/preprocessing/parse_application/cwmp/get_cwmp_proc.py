...
#	$4: -
# 	$5: -
#	$6: error description #full line when error-unknown
# ===================
...
# FIXME
folder_prefix="/home/bano/my_media/nemesis/vertical/07-09-2017/cwmp/"
#folder_prefix="/home/bano/scripts/preprocessing/parse_application/cwmp/"

out_prefix=folder_prefix+"proc/flat/"
cmd='mkdir -p %s' %(out_prefix)
os.system(cmd)

file_input=folder_prefix+"out_cwmp/out.log"
file_output=out_prefix+"cwmp.flat"
file_output_stats=out_prefix+"cwmp.flat.stats"
file_log=out_prefix+"cwmp.flat.log"
...
for line in open(file_input, 'r'):
	...
	try:
		...
		if 'response' in http["data"]["http"] and 'status_code' in http["data"]["http"]["response"]:
                        # Update fields
                        field_label="ok"
			...
			field_statusCode=str(http["data"]["http"]["response"]["status_code"])
				...
				updateStatusStats(field_IP,field_statusCode)
                                ...
                                # Update stats
                                updateStatusStats(field_IP,field_err)
			else:
                                # Update err stats
		...
		fOut.write(field_label+"\t"+field_subLabel+"\t"+field_statusCode+"\t"+field_statusLine+"\t"+field_IP+"\t"+field_err+"\n")
